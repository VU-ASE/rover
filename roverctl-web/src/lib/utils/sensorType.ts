...
const trySensorType = (
  sensorData: TimestampedSensorOutput
...
const getParametersFromFrame = (frame: TimestampedSensorOutput) => {
  ...
  if (frame.sensorData.imuOutput) {
    ...
    entries.push([
      "magnetometerX",
      frame?.sensorData?.imuOutput?.magnetometer?.x ?? 0,
    ]);
    entries.push([
      "magnetometerY",
      frame?.sensorData?.imuOutput?.magnetometer?.y ?? 0,
    ]);
    entries.push([
      "magnetometerZ",
      frame?.sensorData?.imuOutput?.magnetometer?.z ?? 0,
    ]);
    // Add the gyroscope values
    entries.push([
      "gyroscopeX",
      frame?.sensorData?.imuOutput?.gyroscope?.x ?? 0,
    ]);
    entries.push([
      "gyroscopeY",
      frame?.sensorData?.imuOutput?.gyroscope?.y ?? 0,
    ]);
    entries.push([
      "gyroscopeZ",
      frame?.sensorData?.imuOutput?.gyroscope?.z ?? 0,
    ]);
    // Add the accelerometer values
    entries.push([
      "accelerometerX",
      frame?.sensorData?.imuOutput?.accelerometer?.x ?? 0,
    ]);
    entries.push([
      "accelerometerY",
      frame?.sensorData?.imuOutput?.accelerometer?.y ?? 0,
    ]);
    entries.push([
      "accelerometerZ",
      frame?.sensorData?.imuOutput?.accelerometer?.z ?? 0,
    ]);
