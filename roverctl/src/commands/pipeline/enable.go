...
func addEnable(rootCmd *cobra.Command) {
	// General flags
	var roverIndex int
	...
	var infoCmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, enabled := range res.Enabled {
				...
				if enabled.Service.Fq.Author == author && enabled.Service.Fq.Name == name && enabled.Service.Fq.Version == version {
					found = true
				}
			}
			// Add the new service if it wasn't already in the pipeline
			if !found {
				newPipeline = append(newPipeline, *openapi.NewPipelinePostRequestInner(
					openapi.FullyQualifiedService{
						Author:  author,
						Name:    name,
						Version: version,
					},
